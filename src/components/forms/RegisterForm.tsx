  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col gap-6'>
      <div className='flex flex-col gap-1'>
        ...
        <label
          ...
          className={cn(
            ...
            {
              'border-danger-default': errors.email,
      ...
      <div className='flex flex-col gap-1'>
        <p
          className={cn('text-sm leading-[18px] text-dark-default', {
            'text-danger-default': errors.confirmPassword,
            'text-dark-default': !errors.confirmPassword,
          })}
        >
          Confirm password
        </p>
        <label
          ...
          className={cn(
            `relative flex gap-3 bg-transparent items-center px-4 py-3 rounded-lg border border-sec-default`,
            {
              'border-danger-default': errors.confirmPassword,
              'focus-within:border-prim-default label-shadow':
                !errors.confirmPassword,
            }
          )}
        >
          <aside>
            <Image src={lockIcon} alt='Envelope Icon' />
          </aside>
          <input
          ...
          {errors.confirmPassword && (
            <p className='absolute right-3 text-danger-default text-sm leading-4'>{`${errors.confirmPassword.message}`}</p>
          )}
        </label>
      </div>

      <p className='text-sm leading-[18px] text-dark-light self-stretch'>
      ...
      <div className='flex items-center justify-center gap-2 leading-6 text-center w-full'>
        <p className='text-dark-light'>Already have an account? </p>
        <Link href='/login' className='text-prim-default'>
          Login
        </Link>
      </div>
    </form>
